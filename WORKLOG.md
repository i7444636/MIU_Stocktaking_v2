...
- 공동판매 마스터 `입고` 시트의 H열 소분류가 `위탁`인 행을 마스터 비교 대상에서 제외
- 위탁 전용 바코드가 실사 파일에 있어도 `장부부재(신규)`로 분류되지 않도록 실사 비교에서도 제외
- 동일 바코드에 일반 상품 행이 함께 존재하면 일반 상품 데이터는 비교 대상으로 유지
- 업로드 완료 화면에 비교 건수와 위탁 제외 건수를 함께 표시
- 사이트 안내와 README에 위탁 제외 기준 추가
